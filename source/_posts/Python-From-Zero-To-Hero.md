...
`limit` 初始值为 1，每轮循环会增加 1，`while` 内部的语句会一直循环执行，直到 `limit <= 50` 的条件不再满足即 `limit` 大于 50 时终止。`res *= limit` 是 `res = res * limit` 的简写形式，下一行的 `+=` 同理。你可能已经注意到了这段程序计算的是 $50!$ 的值，其结果远远大于 64 位有符号整数所能表示的最大值 ${2^{63}-1}$，但是在 Python 中可以直接计算，因为 Python 原生支持大数运算。
...
`class Student(Person)` 表明 `Student` 类继承自 `Person`，在 `__init__` 中新增了一个 `grade` 参数，并将它的值配给了内部变量 `_grade`。以下划线开头的变量和方法是内部成员，无法被外部访问 ，因此增加了 `get_grade` 方法来获取 `_grade` 的值。
